(function () {
     ...
     * @param {String{ infos.func
     *
     * @param {String|Array} fileColors Optional colors.
     * @param {String|Array} lineColors Optional colors.
     * @param {String|Array} funcColors Optional colors.
     *
     * @return {Object}
     * @return {String} msg             File infos with colors if given
     * @return {int}    msgLength       File infos length without colour elements
     */
    var buildFileInfos = function (infos, fileColors, lineColors, funcColors) {

        var fields = [
            [ infos.filename, fileColors ],
            [ infos.line,     lineColors ],
            [ infos.func,     funcColors ]
        ];
        ...
        fields.forEach(function(field) {
            if(field[0]) {
                msg.push(applyColors(field[0], field[1]));
                msgLen += field[0].toString().length;
            }
        });

        return {
            msg: '[' + msg.join(':') + ']',
            msgLength: (msgLen + 2 + msg.length - 1) // 1 for '[', 1 for ']', 1 for each ':'
        };
    };
     ...
     * @param {Array|String} opt.funcColors          Default colors output for function name. Default undefined.
     *
     ...
     * @param {String}          func        Function. Optional
     */
    Console2.prototype.file = Console2.prototype.f = function (file, line, func) {
    ...
    Console2.prototype.buildContext = function (log, opt) {
        ...
        if (opt.location && log.context.location.filename && log.context.location.line) {
            var infos = buildFileInfos(log.context.location, log.opt.fileColors, log.opt.lineColors, log.opt.funcColors);
    ...
    Console2.prototype.addLogger = function (name, colors, opt) {
        ...
        opt.funcColors       = opt.funcColors || this.opt.funcColors;
